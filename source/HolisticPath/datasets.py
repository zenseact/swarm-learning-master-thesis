...
class ZODImporter:
    def __init__(
        ...
        if stored_balanced_ds_path:
            training_frames_all, validation_frames_all = self.balanced_frames(
                stored_balanced_ds_path
            )
        else:
        ...
        training_frames_all = [
            idx for idx in training_frames_all if self.is_valid_frame(idx)
        ]
        validation_frames_all = [
            idx for idx in validation_frames_all if self.is_valid_frame(idx)
        ]

        self.training_frames = training_frames_all[
            : int(len(training_frames_all) * subset_factor)
        ]
    ...
    def load_datasets(self, num_clients: int):
        ...
        transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Resize((self.img_size, self.img_size))]
        )

        trainset = ZodDataset(
            zod_frames=self.zod_frames,
            frames_id_set=self.training_frames,
        ...
        for ds in datasets:
            ...
            valloaders.append(
                DataLoader(ds_val, batch_size=self.batch_size, num_workers=NUM_WORKERS)
            )
        ...
        completeTrainloader = DataLoader(
            train_split, batch_size=self.batch_size, num_workers=NUM_WORKERS
        )
        completeValloader = DataLoader(
            val_split, batch_size=self.batch_size, num_workers=NUM_WORKERS
        )

        testloader = DataLoader(
            testset, batch_size=self.batch_size, num_workers=NUM_WORKERS
        )
...
def main(
    nr_clients=2, subset_factor=SUBSET_FACTOR, img_size=IMG_SIZE, batch_size=BATCH_SIZE
):
