...
class SwarmSimulator:
    ...
    def wait_until_all_ready(self, clients_network, client_cid_subset=None):
        if (client_cid_subset):
            while (any([not clients_network[c].status == SwarmClient.Status.READY for c in client_cid_subset])):
                time.sleep(2)
        else:
            while (any([not c.status == SwarmClient.Status.READY for c in clients_network])):
                time.sleep(2)

    def simulate_fully_connected_graph(self, nr_clients=NUM_CLIENTS, nr_global_rounds=NUM_GLOBAL_ROUNDS):
    ...
    def simulate_random_dynamic_graph(self, nr_clients=NUM_CLIENTS, nr_global_rounds=NUM_GLOBAL_ROUNDS):
        # create and add clients to clients network
        clients_network = [self.create_client(i) for i in range(nr_clients)]
        ...
        for i in range(nr_global_rounds):
            ...
            # perform one global rounds on all
            self.perform_global_round(clients_network, edges, client_cid_subset=nodes_of_edges, round_number=i)
