...
global_configs_copy = copy.deepcopy(global_configs)


class FixtureTASK(Enum):
    CLASSIFICATION = 1
    REGRESSION = 2


class FixturePartitionStrategy(Enum):
    RANDOM = "random"
    LOCATION = "location"


@dataclass
class FixtureGlobalConfigs:
    def __init__(self):
...
def test_task():
    assert FixtureTASK.CLASSIFICATION.value == TASK.CLASSIFICATION.value
    assert FixtureTASK.REGRESSION.value == TASK.REGRESSION.value


def test_partition_strategy():
    assert FixturePartitionStrategy.RANDOM.value == PartitionStrategy.RANDOM.value
    assert FixturePartitionStrategy.LOCATION.value == PartitionStrategy.LOCATION.value


def test_global_configs():
    fixture_global_configs = FixtureGlobalConfigs()

    fixture_global_configs.now = global_configs_copy.now = 0
    fixture_global_configs.TB_PATH = global_configs_copy.TB_PATH = ""
    fixture_global_configs.ML_TASK = global_configs_copy.ML_TASK = ""

    assert fixture_global_configs.__dict__ == global_configs_copy.__dict__
