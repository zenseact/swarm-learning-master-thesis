...
class FlowerClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        log(f"⤺ Get model parameters of client {self.cid}]")
        return get_parameters(self.net)

    def fit(self, parameters, config):
        log('getting parameters from edge device')
        params = self.edge_com.update_model(self.cid)
        # return 1 as nr of training examples for now
        return params, 1, {}

    def evaluate(self, parameters, config):
        log(f"[Client {self.cid}] evaluate, config: {config}")
        if (not self.net):
            self.net = load_model(self.cid)
        set_parameters(self.net, parameters)
        loss, accuracy = test(self.net, self.valloader)
        if (ML_TASK == TASK.CLASSIFICATION):
            log(f"🌠 [Client {self.cid}] test loss {loss}, accuracy {accuracy}")
            return float(loss), len(self.valloader), {"accuracy": float(accuracy)}
        else:
            log(f"🌠 [Client {self.cid}] test RMSE {loss}")
